...
/**
 * Fetch a session's full decision straight from Didit — same shape as the
 * webhook payload, including presigned S3 URLs for the ID document images
 * (front/back/portrait). Used to show an admin the document images on
 * demand without ever persisting them: Didit regenerates fresh short-lived
 * URLs on every call, so there is nothing worth caching.
...
async function getSessionDecision(sessionId) {
  ...
  if (!res.ok) {
    const text = await res.text().catch(() => '');
    throw new Error(`[didit] decision fetch failed: ${res.status} ${text}`);
  }
...
module.exports = { createVerificationSession, getSessionDecision, verifyWebhookSignature };
